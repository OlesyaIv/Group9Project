...
public class IcrashSystemImpl extends UnicastRemoteObject implements IcrashSystem {
	...
	/**
	 * The current requesting authenticated actor that is performing a method on
	 * the system. This is set by the Actor itself before it performs an oe
	 * action
	 */
	ActAuthenticated currentRequestingAuthenticatedActor;

	/**
	 * The current connected communication company that is performing a method
	 * on the system. This is set by the Actor itself before it performs an oe
	 * action
	 */
	...
	// Messir compositions
	/**
	 * A hashtable of all administrators in the system, stored by their login as
	 * a key.
	 */
	Hashtable<String, CtAdministrator> cmpSystemCtAdministrator = new Hashtable<String, CtAdministrator>();

	/**
	 * A hashtable of all authenticated users in the system, stored by their
	 * login as a key.
	 */
	...
	/**
	 * A hashtable of the humans in the system, stored by their phone number as
	 * a key.
	 */
	Hashtable<String, CtHuman> cmpSystemCtHuman = new Hashtable<String, CtHuman>();

	/**
	 * A hashtable of the actor com companies in the system, stored by their
	 * name as a key.
	 */
	...
	// Messir associations
	/**
	 * A hashtable of the joint alerts and crises in the system, stored by their
	 * alert as a key.
	 */
	Hashtable<CtAlert, CtCrisis> assCtAlertCtCrisis = new Hashtable<CtAlert, CtCrisis>();

	/**
	 * A hashtable of the joint alerts and humans in the system, stored by their
	 * alert as a key.
	 */
	Hashtable<CtAlert, CtHuman> assCtAlertCtHuman = new Hashtable<CtAlert, CtHuman>();

	/**
	 * A hashtable of the joint class types and actors in the system, stored by
	 * their class type as a key.
	 */
	...
	/**
	 * A hashtable of the joint class coordinators and actors coordinators in
	 * the system, stored by their class type.
	 */
	Hashtable<CtCoordinator, ActCoordinator> assCtCoordinatorActCoordinator = new Hashtable<CtCoordinator, ActCoordinator>();

	/**
	 * A hashtable of the joint crises and coordinators in the system, stored by
	 * their crisis as a key.
	 */
	Hashtable<CtCrisis, CtCoordinator> assCtCrisisCtCoordinator = new Hashtable<CtCrisis, CtCoordinator>();

	/**
	 * A hashtable of the joint humans and Actor com companies in the system,
	 * stored by the human as a key.
	 */
	...
	/** Current using quality dataset. */
	DtQualityDataset currentQualityDataset = new DtQualityDataset(new PtReal(0.0), new PtReal(0.0), new PtReal(0.0),
			new PtInteger(0));
	 ...
	 *
	 * @param aCtCrisis
	 *            The crisis to use to search for the associated alert
	...
	/**
	 * Gets the class Authenticated (Of a coordinator type) that has the
	 * associated ID provided.
	 *
	 * @param aDtCoordinatorID
	 *            The ID to use to search for the user
	 * @return the class Authenticated that was associated with the ID provided,
	 *         if none found then return a null
	 */
	 ...
	 *
	 * @param aActAuthenticated
	 *            The actor to use to search for the class Authenticated
	 * @return the class Authenticated that was associated with the Actor
	 *         provided, if none found then return a null
	 */
	private CtAuthenticated getCtAuthenticated(ActAuthenticated aActAuthenticated) {
		try {
			...
			for (CtAuthenticated ctAuth : assCtAuthenticatedActAuthenticated.keySet()) {
				...
				if (currActAuth.getLogin().value.getValue().equals(ActAuthname)
						&& !currActAuth.getLogin().value.getValue().isEmpty())
					return ctAuth;
			}
	...
	/**
	 * A standard check to see if the system is started, if not then an
	 * exception is thrown to be caught by the checking method.
	 *
	 * @throws Exception
	 *             The exception with a simple text description of the reason
	 *             for failure
	 */
	...
	/**
	 * A standard check to see if the current actor requesting to do an action
	 * on the server is logged in and also an admin.
	 *
	 * @throws Exception
	 *             The exception with a simple text description of the reason
	 *             for failure
	...
	private void isUserLoggedIn() throws Exception {
		...
		if (user != null && user.value != null) {
			CtAuthenticated ctAuthenticatedInstance = cmpSystemCtAuthenticated.get(user.value.getValue());
	...
	/**
	 * If an exception is thrown inside a lambda stream, it must be caught
	 * during the actual stream. This is a method to catch said issue and then
	 * propagate it up to the calling method. See here for more information:
	 * http://stackoverflow.com/questions/19757300/java-8-lambda-streams-filter-by-method-with-exception
	 *
	 * @param <V>
	 *            the value type
	 * @param callable
	 *            the lambda stream that will cause the exception. It's called
	 *            like this a -&gt; propagate(a::isActive) where isActive is a
	 *            method inside the class of a
	 * @return Returns the result if completed, otherwise it will throw the
	 *         exception up to the calling method
	 */
	...
	/**
	 * Creates a wrapper around an exception to prevent Java compile errors when
	 * attempting to catch an exception inside a lambda expression.
	 *
	 * @param e
	 *            The exception to throw to the surrounding method
	 ...
	 *             If the server is not alive, this exception will be thrown
	 */
	...
	/**
	 * Instantiates a new iCrash system implementation. Constructor of the class
	 * 
	 * @throws RemoteException
	 *             If the server is not alive, this exception will be thrown
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * setCurrentRequestingAuthenticatedActor(lu.uni.lassy.excalibur.examples.
	 * icrash.dev.java.environment.actors.ActAuthenticated)
	 */
	synchronized public void setCurrentRequestingAuthenticatedActor(ActAuthenticated aActAuthenticated)
			throws RemoteException {
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * getCurrentRequestingAuthenticatedActor()
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * setCurrentConnectedComCompany(lu.uni.lassy.excalibur.examples.icrash.dev.
	 * java.environment.actors.ActComCompany)
	 */
	synchronized public void setCurrentConnectedComCompany(ActComCompany aComCompany) throws RemoteException {
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * getRandomCtCoordinator()
	 */
	public CtCoordinator getRandomCtCoordinator() throws RemoteException {
		List<CtCoordinator> collCtCoor = assCtCoordinatorActCoordinator.keySet().stream().filter(t -> t != null)
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * getActCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.CtCoordinator)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * bindCtCrisisCtCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java
	 * .system.types.primary.CtCrisis,
	...
	public List<ActAdministrator> getAllActAdministrators() throws RemoteException {
		...
		for (CtAuthenticated ctAuth : assCtAuthenticatedActAuthenticated.keySet()) {
			if (ctAuth instanceof CtAdministrator)
				listAdmins.add((ActAdministratorImpl) assCtAuthenticatedActAuthenticated.get(ctAuth));
		}
		return listAdmins;
	}
	...
	public ArrayList<CtAdministrator> getAllCtAdministrators() throws RemoteException {
		...
		for (CtAuthenticated ctAuth : cmpSystemCtAuthenticated.values()) {
			if (ctAuth instanceof CtAdministrator)
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * getActComCompany(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.CtHuman)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * getActComCompany(java.lang.String)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeCreateSystemAndEnvironment(lu.uni.lassy.excalibur.examples.icrash.dev.
	 * java.types.stdlib.PtInteger)
	 */
	public PtBoolean oeCreateSystemAndEnvironment(PtInteger aQtyComCompanies) throws RemoteException {
		try {
			 ...
			 * PostF 1:
			 * 
			 * the ctState instance is initialised with the integer 1 for the
			 * crisis and alert counters used for their identifications, clock =
			 * current time the crisis reminder period is set to 300 seconds,
			 * the maximum crisis reminder period is fixed to 1200 seconds (i.e.
			 * 20 minutes) and the system is considered in a started state.
			 * aVpLastReminder = current time Those predicates must be satisfied
			 * first since all the other depend on the existence of a ctState
			 * instance !
			 */
			...
			try {
				String strReminderPeriod = PropertyUtils.getInstance().getProperty("iCrash.reminderPeriod",
						String.valueOf(300));
				String strMaxReminderPeriod = PropertyUtils.getInstance().getProperty("iCrash.maxReminderPeriod",
						String.valueOf(1200));
				...
				DtDate aDtDate = ICrashUtils.stringToDtDate(PropertyUtils.getInstance().getProperty("iCrash.date",
						ICrashUtils.getMinimumDateAndTime().date.toString()));
				DtTime aDtTime = ICrashUtils.stringToDtTime(PropertyUtils.getInstance().getProperty("iCrash.time",
						ICrashUtils.getMinimumDateAndTime().time.toString()));
			...
			ctState.init(aNextValueForAlertID, aNextValueForCrisisID, aClock, aCrisisReminderPeriod,
					aMaxCrisisReminderPeriod, aClock, aVpStarted);
			/*
			 * ENV PostF 2 the actMsrCreator actor instance is initiated
			 * (remember that since the oeCreateSystemAndEnvironment is a
			 * special event, its role is to make consistent the post state,
			 * thus creating the actor and its interfaces is required even
			 * though the sending of this message logically would need the actor
			 * and its interfaces to already exist ...).
			 */
			// implementation done at the level of the init method caller

			/*
			 * ENV PostF 3 the environment for communication company actors, in
			 * the post state, is made of AqtyComCompanies instances allowing
			 * for receiving and sending messages to humans.
			 */
			...
			/*
			 * ENV PostF 4 the environment for administrator actors, in the post
			 * state, is made of one instance.
			 */
			...
			/*
			 * ENV PostF 5 the environment for activator actors, in the post
			 * state, is made of one instance allowing for automatic message
			 * sending based on current system’s and environment state’.
			 */
			ActActivator aActActivator = new ActActivatorImpl();
			env.setActActivator(aActActivator);
			/*
			 * PostF 6 the set of ctAdministrator instances at post is made of
			 * one instance initialized with ’icrashadmin’ (resp.
			 * ’7WXC1359’) for login (resp. password) values.
			 */
			...
			DtPhoneNumber aPhn = new DtPhoneNumber(new PtString("+79999999999"));
			ctAdmin.init(aLogin, aPwd, aPhn);
			/*
			 * PostF 7 the association between ctAdministrator and
			 * actAdministrator is made of one couple made of the jointly
			 * specified instances.
			 */
			...
			/*
			 * Creating a thread to auto check if handling delay has passed and
			 * if so run oeSollicitateCrisisHandling
			 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeAlert(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.
	 * primary.EtHumanKind,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.types.stdlib.DtDate,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.types.stdlib.DtTime,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtPhoneNumber,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtGPSLocation,
	 ...
	 * DtComment)
	 */
	public synchronized PtBoolean oeAlert(EtHumanKind aEtHumanKind, DtDate aDtDate, DtTime aDtTime,
			DtPhoneNumber aDtPhoneNumber, DtGPSLocation aDtGPSLocation, DtComment aDtComment) throws RemoteException {
		try {
			// PreP1
			isSystemStarted();
			...
			// check if there already exists a reported Alert that is closer
			// than 100 m.
			for (CtAlert existingCtAlert : assCtAlertCtCrisis.keySet()) {
				existsNear = existingCtAlert.location.isNearTo(aDtGPSLocation.latitude, aDtGPSLocation.longitude)
						.getValue();
				if (existsNear) {
			...
			// if there no exits a near alert, then we need to initialise the
			// just created crisis instance
			if (!existsNear) {
			...
			// DB: update just inserted alert with its corresponding associated
			// (near) crisis
			...
			// if there no exists human, then we need (1) to initialise the just
			// created instance
			...
			if (!existsHuman) {
				...
				String idComCompany = DbComCompanies.getComCompanyID(currentConnectedComCompany.getName());
			...
			// PostF6
			DtSMS sms = new DtSMS(
					new PtString("Your alert has been registered. We will handle it and keep you informed."));
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeValidateAlert(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtAlertID)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeInvalidateAlert(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtAlertID)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeSetCrisisType(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtCrisisID,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * EtCrisisType)
	 */
	public synchronized PtBoolean oeSetCrisisType(DtCrisisID aDtCrisisID, EtCrisisType aEtCrisisType)
			throws RemoteException {
		try {
			// PreP1
			isSystemStarted();
			...
			if (currentRequestingAuthenticatedActor instanceof ActCoordinator) {
				...
				PtString aMessage = new PtString("The crisis with ID '" + aDtCrisisID.value.getValue()
						+ "' is now of type '" + aEtCrisisType.toString() + "' !");
				theActCoordinator.ieMessage(aMessage);
				return new PtBoolean(true);
			}
		} catch (Exception e) {
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeSetCrisisStatus(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtCrisisID,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * EtCrisisStatus)
	 */
	public synchronized PtBoolean oeSetCrisisStatus(DtCrisisID aDtCrisisID, EtCrisisStatus aEtCrisisStatus)
			throws RemoteException {
		try {
			// PreP1
			isSystemStarted();
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeSetCrisisHandler(lu.uni.lassy.excalibur.examples.icrash.dev.java.system
	 * .types.primary.DtCrisisID)
	 */
	public synchronized PtBoolean oeSetCrisisHandler(DtCrisisID aDtCrisisID) throws RemoteException {
		try {
			// PreP1
			isSystemStarted();
			// PreP2
			...
			if (currentRequestingAuthenticatedActor instanceof ActCoordinator) {
				...
				if (theCtCoordinatorAtPre == null)
					...
					log.debug("theCtCoordinatorAtPre is " + assCtCrisisCtCoordinator.get(theCrisis).toString());
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeReportOnCrisis(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtCrisisID,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtComment)
	 */
	public synchronized PtBoolean oeReportOnCrisis(DtCrisisID aDtCrisisID, DtComment aDtComment) {
		try {
			// PreP1
			isSystemStarted();
			// PreP2
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeGetCrisisSet(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.EtCrisisStatus)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeGetAlertsSet(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.EtAlertStatus)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeCloseCrisis(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtCrisisID)
	 */
			...
			if (currentRequestingAuthenticatedActor instanceof ActCoordinator) {
				...
				for (int i = 0; i < alertkeys.length; i++) {
					...
					if (assCtAlertCtCrisis.get(theAlert) == theCrisis) {
						...
						human = assCtAlertCtHuman.get(theAlert);
				...
				try {
					...
					IcrashSMSWindow wnd = new IcrashSMSWindow(currentQualityDataset);
					wnd.setVisible(true);
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeLogin(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.
	 * primary.DtLogin,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtPassword)
	 */
	...
	public PtBoolean oeLogin(DtLogin aDtLogin, DtPassword aDtPassword) throws RemoteException {
		try {
			log.debug("The current requesting authenticating actor is "
					+ currentRequestingAuthenticatedActor.getLogin().value.getValue());
			// PreP1
			isSystemStarted();
			/**
			 * check whether the credentials corresponds to an existing user
			 * this is done by checking if there exists an instance with such
			 * credential in the ctAuthenticatedInstances data structure
			 */
			...
			if (ctAuthenticatedInstance != null) {
				...
				if (pwdCheck.getValue()) {
					// PostP1
					/**
					 * Make sure that the user logging in is the current
					 * requesting user We do this as each window is a dumb
					 * terminal and only one use can logon at each individual
					 * window So user 1 can only logon at the window for user 1,
					 * if user 2 tries, it should fail
					 */
					...
					if (authActorCheck != null && authActorCheck.getLogin().value.getValue()
							.equals(currentRequestingAuthenticatedActor.getLogin().value.getValue())) {
						// ctAuthenticatedInstance.vpIsLogged = new
						// PtBoolean(true);
						// PostF1
						// PtString aMessage = new PtString("You are logged !
						// Welcome ...");
						// currentRequestingAuthenticatedActor.ieMessage(aMessage);
						Random rnd = new Random(System.currentTimeMillis());
						PtString code = new PtString(Integer.toString(1000 + rnd.nextInt(8999 + 1)));
						currentRequestingAuthenticatedActor.setCode(new DtPhoneCode(code));
						// current.code = new DtPhoneCode(code);
						DtSMS sms = new DtSMS(
								new PtString("You received this message from ICrash system. To login use this code: "
										+ code.getValue()));
						// DtPhoneNumber phone = new DtPhoneNumber(new
						// PtString("89675354752"));
						getAllActComCompanies().get(0).ieSmsSend(ctAuthenticatedInstance.phn, sms);
						return new PtBoolean(true);
					}
				}
			}
	...
	public PtBoolean oeLoginByCode(DtLogin aDtLogin, DtPhoneCode aDtCode) {
		try {
			log.debug("before all");
			CtAuthenticated ctAuthenticatedInstance = cmpSystemCtAuthenticated.get(aDtLogin.value.getValue());
			log.debug("before inactorcheck");
			ActAuthenticated authActorCheck = assCtAuthenticatedActAuthenticated.get(ctAuthenticatedInstance);
			if (authActorCheck != null && authActorCheck.getLogin().value.getValue()
					.equals(currentRequestingAuthenticatedActor.getLogin().value.getValue())) {
				log.debug("before authCheck " + authActorCheck.getCode().value.getValue());
				if (aDtCode.value.getValue().equals(authActorCheck.getCode().value.getValue())) {
					// log.debug("before vp");
					ctAuthenticatedInstance.vpIsLogged = new PtBoolean(true);
					// log.debug("after vp");
					PtString aMessage = new PtString("You are logged ! Welcome ...");
					currentRequestingAuthenticatedActor.ieMessage(aMessage);
					// log.debug("after message");
					return new PtBoolean(true);
				}
			}
		} catch (RemoteException e) {
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeAddCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.
	 * types.primary.DtCoordinatorID,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtLogin,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtPassword)
	...
	public PtBoolean oeAddCoordinator(DtCoordinatorID aDtCoordinatorID, DtLogin aDtLogin, DtPassword aDtPassword,
			DtPhoneNumber aDtPhoneNumber) throws RemoteException {
		try {
			// PreP1
			isSystemStarted();
			// PreP2
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeDeleteCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.
	 * system.types.primary.DtCoordinatorID)
	 */
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeUpdateCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.
	 * system.types.primary.DtCoordinatorID,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 * DtLogin,
	 * lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.
	 ...
	 */
	/*
	 * This system operation is not specified in the Messir Analysis Document.
	 * However, such functionality has been implemented to show how a
	 * coordinator can change his login credentials. It is worth noticing that
	 * such system operation is not used anywhere for the moment (not even
	 * included in the class' interface)
	 * 
	 */
	public PtBoolean oeUpdateCoordinator(DtCoordinatorID aDtCoordinatorID, DtLogin aDtLogin, DtPassword aDtPassword,
			DtPhoneNumber aDtPhoneNumber) throws java.rmi.RemoteException {
		try {
			// PreP1
			isSystemStarted();
			// PreP2
		...
		try {
			...
			for (String crisisKey : cmpSystemCtCrisis.keySet()) {
				...
				// PostF2
					...
					// notify all ActAuthenticated about crises not being
					// handled
	...
	/**
	 * A check to see if the handling delay has passed the standard set in
	 * CtState.
	 *
	 * @throws Exception
	 *             the exception that was encountered when trying to run this
	 *             method
	 */
	private synchronized void hasHandlingDelayPassed() throws Exception {
		// Interesting read:
		// http://stackoverflow.com/questions/19757300/java-8-lambda-streams-filter-by-method-with-exception
		if (cmpSystemCtCrisis.values().stream()
				.filter(t -> t.status == EtCrisisStatus.pending && propagate(t::handlingDelayPassed).getValue())
				.count() == 0)
	 ...
	 * 
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#
	 * oeSetClock(lu.uni.lassy.excalibur.examples.icrash.dev.java.types.stdlib.
	 * DtDateAndTime)
	 */
	public PtBoolean oeSetClock(DtDateAndTime aCurrentClock) {
		try {
			...
			if (aCurrentClock.toSecondsQty().getValue() <= ctState.clock.toSecondsQty().getValue()) {
				log.error("The clock of " + aCurrentClock.toString() + " is less than the current clock of "
