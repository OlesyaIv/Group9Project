 ...
 */
/**
 * The Class ICrashGUIController, which deals with handling the GUI and it's
 * functions for the Administrator.
 */
public class ICrashAdminGUIController extends AbstractAuthGUIController {

	/*
	 * This section of controls and methods is to be replaced by modifications
	 * in the ICrash.fxml document from the sample skeleton controller When
	 * replacing, remember to reassign the correct methods to the button event
	 * methods and set the correct types for the tableviews
	 */
	 ...
	 */
	@FXML
	void bttnBottomAdminShowStatistics_OnClick(ActionEvent event) throws ServerOfflineException, ServerNotBoundException {
		showChart(true);
	}
					...
					try {
						...
						case Add:
							if (userController.oeAddCoordinator(txtfldUserID.getText(), txtfldUserName.getText(),
									psswrdfldPassword.getText(), txtfldUserPhone.getText()).getValue()) {
		...
		if (!loginChecked) {
			if (txtfldAdminUserName.getText().length() > 0 && psswrdfldAdminPassword.getText().length() > 0) {
				try {
					...
					if (userController.oeLogin(txtfldAdminUserName.getText(), psswrdfldAdminPassword.getText())
							.getValue()) {
	...
	public boolean checkCode() throws ServerOfflineException {
		...
		try {
			if (userController.oeLoginByCode(txtfldAdminUserName.getText(), txtfldPhoneCode.getText()).getValue()) {
				return true;
			}
	...
	public void showChart(boolean parameter) throws ServerOfflineException, ServerNotBoundException {

		if (parameter) {
			DtQualityDataset dataset = userController.getQualityDataset();
			iCrashAdminStatChart chart = new iCrashAdminStatChart("Satisfaction statistics", "Satisfaction statistics", dataset);
			chart.pack();
